...
{%- set mip = {'val':''} %}
{%- set dsvr = m.get('software').get('mysql').get('deploy server','') %}
{%- for psvr in m.get('logical server').get(dsvr).get('physical server',[]) %}
{%- if "mysql_master" in m.get('physical server').get(psvr).get('role',[]) %}
{%- do mip.update({'val':m.get('physical server').get(psvr).get(cfg.iptype)}) %}
{%- endif %}
{%- endfor %}
